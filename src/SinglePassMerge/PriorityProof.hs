...
merge (Node p1 lRank p1gep l1 r1) (Node p2 rRank p2gep l2 r2) =
  case (order p1 p2, rank l1 >= rank r1 + rRank, rank l2 >= rank r2 + lRank ) of
    (Le p1lep2, True , _) -> Node p1 (lRank + rRank) p1gep l1 (merge r1 (Node p2 rRank p1lep2 l2 r2))
    (Le p1lep2, False, _) -> Node p1 (lRank + rRank) p1gep (merge r1 (Node p2 rRank p1lep2 l2 r2)) l1
    (Ge p2lep1, _, True ) -> Node p2 (lRank + rRank) p2gep l2 (merge (Node p1 lRank p2lep1 l1 r1) r2)
    (Ge p2lep1, _, False) -> Node p2 (lRank + rRank) p2gep (merge (Node p1 lRank p2lep1 l1 r1) r2) l2
